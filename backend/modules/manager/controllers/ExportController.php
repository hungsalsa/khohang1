...
use backend\modules\manager\models\Employee;
use backend\modules\manager\models\Product;
use backend\modules\manager\models\Inventory;
use backend\modules\manager\models\ExportSearch;
...
class ExportController extends Controller
{
    ...
    {
        $model = new Export();
        $modelIn = new Inventory();

        $employee = new Employee();
        $allEmployee = ArrayHelper::map($employee->getAllEmployee(),'id_em','emName');

        $product = new Product();
        $allProduct = ArrayHelper::map($product->getAllProduct(),'idPro','proName');

        $model->created_at = date('Y-m-d');
        $model->updated_at = date('Y-m-d');

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->idEx]);
